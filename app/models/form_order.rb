class FormOrder 
  ...
  with_options presence: true do
    validates :user_id
    validates :item_id
    validates :delivery_zipcode, format: { with: /\A\d{3}-\d{4}\z/, message: "Postal code is invalid.Enter it as follows (e.g. 123-4567)" }
    ...
    validates :delivery_phone, length: { in: 10..11, too_short: "Phone number is too short" }, format: { with: /\A\d+\z/, message: "Phone number is invalid. Input only number" }
    validates :token
  end
